...
export function getRegion(pokemonId: number): string | null {
  for (const region of REGIONS) {
    if (pokemonId >= region.startId && pokemonId <= region.endId) {
      return region.name;
    }
  }
  return null;
...
export function getRegionDisplayName(pokemonId: number): string {
  for (const region of REGIONS) {
    if (pokemonId >= region.startId && pokemonId <= region.endId) {
